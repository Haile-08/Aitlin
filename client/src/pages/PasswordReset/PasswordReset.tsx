...
import { passwordReset } from '../../hook/authHook';
import { useMutation } from 'react-query';
import { resetDataTypes } from '../../@types';
    ...
    <div className="w-dvw h-dvh flex justify-center items-center flex-row">
      ...
      <img src={logo} alt="logo" className='mb-20 cursor-pointer' onClick={handleHomeNav}/>
        <div className="w-10/12 flex flex-col">
          <p className='text-3xl font-roboto md:font-semibold mb-10'>Nueva contraseña</p>
          <form className='flex flex-col' onSubmit={handleSubmit(onSubmit)}>
            ...
            <label htmlFor="password" className="mt-6 block font-roboto md:font-light mb-2 dark:text-white">confirmar Contraseña</label>
            <div className={`relative h-14 rounded-lg border-2 ${errors.password? 'border-error' : 'border-primary-color'} flex flex-row justify-start items-center`}>
              <i className='absolute ml-6 mr-6 '><img src={passwordLogo} alt="password logo" /></i>
              <div className="absolute left-16 h-2/3 w-0.5 bg-black opacity-25"></div>
              ...
              <i onClick={()=> setIsVisbleComfirm(!isVisbleComfirm)} className='mr-6'> 
                 {isVisbleComfirm ? <img src={show} alt="show password" className='w-8' />: <img src={hide} alt="show password" className='w-8'/>}
              </i>
            </div>
