...
function Binnacle() {
    ...
    const [setIsOpen,isOpen, filterBool, id, _billArchive, blogArchive, _nurseArchive] = useOutletContext() as OutletContextType;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
      <>
          <div className="w-[95%] h-[15%] md:h-[10%]  flex justify-between items-center flex-row">
            ...
            <button onClick={handleDownload} className="flex bg-primary-color font-roboto text-white justify-center items-center ml-1 md:ml-2 px-0 md:px-3 py-0 text-base rounded-md">
                <p className="hidden md:block p-0 m-0">Descargar</p>
                <img src={downloadIcon} alt="add" className="flex md:hidden py-2"/>
              </button>
            <button className="flex bg-primary-color font-roboto text-white justify-center items-center ml-1 md:ml-2 px-0 md:px-3 py-0 text-base rounded-md" onClick={()=> setIsOpen(true)}>
              <p className="hidden md:block p-0 m-0">Agregar bitácora</p>
          ...
          {isLoading && array.map((_item, index)=>(
           ...
           ))}
            {
              ...
              data?.data.map((binnacle: any)=>(
                <BinnacleList Name={binnacle?.Name} refetch={refetch} deleteItem={deleteItem} period={binnacle?.period} comment={binnacle?.comment} id={binnacle._id} link={binnacle?.files}/>
              ))
            }
            </div>
      </>
